...
export const guardTransactions = async (
  ...
  const signatures = result.data.transactions.map((t: { guardianSignature: string }) => t.guardianSignature);
...
export const requiresGuardianSignature = (tx: Transaction) => {
  return (
    tx.getOptions().isWithGuardian() && tx.getGuardian().bech32().length > 0 && tx.getGuardianSignature().length === 0
  );
};
...
export const shouldApplyGuardianSignature = (providerType: AuthProviderType) => {
  ...
  return [AuthProviderType.LEDGER, AuthProviderType.PEM, AuthProviderType.WALLET_CONNECT].includes(providerType);
};
...
export const computeNetworkConfig = (config: NetworkEnv | NetworkOptions): RequiredNetworkOptions => {
  if (typeof config === "string") {
    ...
    return {
      ...
      xAliasUrl: walletUrls[config].xAliasAddress,
    };
  }
  ...
  return {
    ...
    xAliasUrl: walletUrls[env].xAliasAddress,
    ...config,
  };
};
