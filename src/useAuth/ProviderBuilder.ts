...
import { XAliasProviderFactory } from "@elrond-giants/erdjs-auth/dist/Factories";
...
export default class ProviderBuilder implements IProviderBuilder {
  ...
  protected buildXaliasProvider() {
    return new XAliasProviderFactory({
      chainId: this.options.chainId,
      networkOptions: this.options.webConnectionOptions,
      walletAddress: this.options.xAliasAddress,
    }).createProvider();
  }
