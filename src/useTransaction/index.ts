...
export const useTransaction = () => {
  ...
  const whenCompleted = (txHash: string, options: IPoolingOptions = {}): Promise<ITransactionOnNetwork> => {
  ...
  const sendWebTransaction = async (tx: Transaction, returnUrl: string) => {
    ...
    let urlString =
      `receiver=${plainTx.receiver}&value=${plainTx.value}` +
